...
export class AstryxThemeConnector implements ThemeConnector {
  ...
  readonly components: ThemeComponents = createBaseThemeComponents({
    systemId: "astryx",
    card: {
      container: (props) => `astryx-card p-6 rounded-xl border border-solid border-[var(--astryx-border,#e2e8f0)] bg-[var(--astryx-surface,#ffffff)] shadow-sm transition-all hover:shadow-md ${props.class || ""}`.trim(),
      title: "text-1.25rem font-header font-bold mb-3 color-[var(--astryx-text,#0f172a)] tracking-tight",
      content: "color-[var(--astryx-text-muted,#475569)] text-0.95rem leading-relaxed",
    },
    button: (props) => `inline-flex items-center justify-center px-5 py-2.5 rounded-lg font-medium text-sm transition-all cursor-pointer border-0 bg-[var(--astryx-primary,#1877f2)] text-white hover:bg-[var(--astryx-primary-hover,#166fe5)] active:scale-98 shadow-sm ${props.class || ""}`.trim(),
    grid: (props) => `grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-6 my-8 ${props.class || ""}`.trim(),
    hero: {
      container: "relative min-h-[420px] flex items-center justify-center text-center my-10 p-12 rounded-2xl bg-gradient-to-b from-[var(--astryx-surface-variant,#f8fafc)] to-[var(--astryx-surface,#ffffff)] border border-solid border-[var(--astryx-border,#e2e8f0)] shadow-sm overflow-hidden",
      title: "text-3rem md:text-4.2rem font-header font-extrabold mb-5 color-[var(--astryx-text,#0f172a)] tracking-tight leading-tight",
      subtitle: "text-1.25rem font-body text-[var(--astryx-text-muted,#475569)] leading-relaxed max-w-2xl mx-auto",
      background: (imageUrl) => <div class="absolute inset-0 bg-cover bg-center opacity-15" style={{ backgroundImage: `url('${imageUrl}')` }}></div>,
    },
    image: {
      container: (props) => [
      ...
      caption: "text-center color-[var(--astryx-text-muted,#475569)] text-0.8rem mt-2",
    },
    codeBlock: {
      container: () => "my-6 rounded-xl overflow-hidden border border-solid border-[#334155] bg-[#0f172a] text-[#f8fafc] p-5 font-mono text-0.875rem shadow-md",
      title: "text-xs text-[#94a3b8] border-b border-solid border-[#334155]/60 pb-2.5 mb-3 font-mono tracking-wide flex items-center gap-2",
      content: "m-0 overflow-x-auto text-[#f8fafc]",
    },
    table: {
      container: "my-6 overflow-x-auto rounded-xl border border-solid border-[var(--astryx-border,#e2e8f0)] shadow-sm",
    ...
    quote: {
      container: "my-6 p-6 rounded-r-xl border-l-4 border-solid border-[var(--astryx-primary,#1877f2)] bg-[var(--astryx-surface-variant,#f8fafc)] italic color-[var(--astryx-text,#0f172a)] shadow-sm",
      content: "m-0 text-1.1rem leading-relaxed font-body",
      caption: "block text-right text-xs font-medium not-italic text-[var(--astryx-text-muted,#475569)] mt-3 uppercase tracking-wider",
    },
    video: {
      container: "my-6 rounded-xl overflow-hidden border border-solid border-[var(--astryx-border,#e2e8f0)] shadow-sm",
      wrapper: "aspect-video w-full bg-[var(--astryx-surface-variant,#f8fafc)]",
      caption: "text-center text-0.8rem text-[var(--astryx-text-muted,#475569)] mt-2 italic px-4 pb-3",
    },
    delimiter: "border-t border-solid border-[var(--astryx-border,#e2e8f0)] w-full my-8",
    nav: {
      container: "flex gap-6 items-center max-lg:fixed max-lg:top-0 max-lg:right-0 max-lg:h-[100dvh] max-lg:w-280px max-lg:bg-[var(--astryx-surface,#ffffff)] max-lg:flex-col max-lg:items-start max-lg:pt-100px max-lg:p-8 max-lg:gap-6 max-lg:border-l max-lg:border-l-solid max-lg:border-l-[var(--astryx-border,#e2e8f0)] max-lg:translate-x-full max-lg:invisible max-lg:transition-all max-lg:duration-300 max-lg:ease-in-out max-lg:z-1000 max-lg:overflow-y-auto lg:hidden",
    ...
    header: {
      container: "sticky top-0 z-2000 border-b border-b-solid border-[var(--astryx-border,#e2e8f0)] bg-[var(--astryx-surface,#ffffff)]/85 backdrop-blur-12px shadow-xs",
      inner: "max-w-1200px mx-auto flex justify-between items-center px-[clamp(1rem,5vw,2.5rem)] py-3.5",
      brand: "font-header text-[var(--astryx-text,#0f172a)] no-underline text-1.25rem font-bold tracking-tight flex items-center gap-2.5",
      renderNavArea: (props) => (
        <>
    ...
    footer: {
      container: "border-t border-t-solid border-[var(--astryx-border,#e2e8f0)] px-[clamp(1rem,5vw,2.5rem)] py-14 mt-20 bg-[var(--astryx-surface-variant,#f8fafc)] flex-shrink-0",
      ...
      navContainer: "flex flex-wrap gap-x-8 gap-y-4 justify-center md:justify-end flex-1",
